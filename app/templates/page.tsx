        ...
        <div className="mx-auto grid w-full max-w-7xl gap-10 px-5 py-16 sm:px-6 lg:grid-cols-[0.88fr_1.12fr] lg:px-8 lg:py-20">
          <div>
            ...
            <div className="mt-8 flex flex-col gap-3 sm:flex-row">
              <Link
                href="/login"
                className="inline-flex items-center justify-center bg-cyan-300 px-6 py-3 text-sm font-bold text-foreground transition hover:bg-cyan-200"
              >
                Start free
              </Link>
              <a
                href="#template-grid"
                className="inline-flex items-center justify-center border border-border bg-surface px-6 py-3 text-sm font-bold text-foreground transition hover:border-border hover:bg-surface"
              >
                Browse templates
              </a>
            </div>
      ...
      >
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
          {CAMPAIGN_TEMPLATES.map((template) => (
            <article
              key={template.slug}
              className="flex min-h-full flex-col border border-border bg-surface p-5 transition hover:border-border hover:bg-surface"
            >
              <div className="mb-5">
              ...
              <div className="mt-auto grid gap-2 pt-6">
                <Link
                  ...
                  className="inline-flex w-full items-center justify-center border border-border bg-surface px-4 py-3 text-sm font-bold text-foreground transition hover:border-border hover:bg-surface"
                >
                  View playbook
                </Link>
                <Link
                  href={`/login?template=${template.slug}`}
                  className="inline-flex w-full items-center justify-center bg-cyan-300 px-4 py-3 text-sm font-bold text-foreground transition hover:bg-cyan-200"
                >
                  Use this template
                </Link>
              </div>
